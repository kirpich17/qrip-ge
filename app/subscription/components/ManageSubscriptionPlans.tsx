...
export default function PlanSelection() {
  ...
  const [planPromoStates, setPlanPromoStates] = useState<
    Record<string, PlanPromoState>
  >({});
  ...
  useEffect(() => {
    if (preselectedPlanId && plans && !toastShownRef.current) {
      ...
      toast.success(
        `Plan preselected! You can proceed with payment or choose a different plan.`
      );
    ...
    if (!memorialId) {
      try {
        ...
        setPlanPromoStates((prev) => ({
          ...
          [planId]: {
            ...state,
            error: 'Could not save promo locally. Try again later.',
          },
        }));
      }
      return;
  ...
  useEffect(() => {
    ...
    if (preselectedPlanId && promoFromQuery) {
      ...
      setPlanPromoStates((prev) => ({
        ...prev,
        [preselectedPlanId]: {
          ...(prev[preselectedPlanId] || {
            input: '',
            appliedPromo: null,
            error: '',
            isValidating: false,
            selectedDuration:
              plans.find((p) => p._id === preselectedPlanId)?.defaultDuration ||
              '1_month',
          }),
    ...
    plans.forEach((plan) => {
      const stored =
        typeof window !== 'undefined'
          ? localStorage.getItem(`pendingPromoCode_${plan._id}`)
          : null;
      if (stored && !promoAppliedRef.current) {
        ...
        setTimeout(() => {
          ...
          try {
            localStorage.removeItem(`pendingPromoCode_${plan._id}`);
          } catch (_) {}
        }, 100);
      }
    });
  ...
  const calculateDiscountedPrice = (plan: Plan) => {
    ...
    if (
      !state ||
      !state.appliedPromo ||
      !doesPromoApplyToPlan(plan._id, state.appliedPromo)
    ) {
      const selectedDurationOption = plan.durationOptions?.find(
        (opt) =>
          opt.duration ===
            (state?.selectedDuration || plan.defaultDuration || '1_month') &&
          opt.isActive
      );
    ...
    const selectedDurationOption = plan.durationOptions?.find(
      (opt) => opt.duration === state.selectedDuration && opt.isActive
    );
  ...
  const handleSelectPlan = async (planId: string) => {
    ...
    try {
      ...
      if (plan.planType === 'minimal' && memorialId) {
        ...
        try {
          ...
          if (memorialHasVideos) {
            console.log(
              '❌ Blocking minimal plan - videos detected (from state)'
            );
            toast.error(
              'Video upload feature is available for premium memorials. Please select a Premium plan to proceed.',
              {
      ...
      if (response.data.redirectUrl) {
        ...
        toast.error(
          translations?.paymentError?.initiate || 'Failed to initiate payment'
        );
      }
    } catch (err: any) {
      toast.error(
        err.response?.data?.message ||
          translations?.paymentError?.process ||
  ...
  if (error) {
    return (
      <div className="py-10 text-red-500 text-center">
        {translations?.error || 'Error loading plans'}
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto px-4 max-w-6xl">
        ...
        <div className="mb-8">
          <div className="bg-green-50 p-4 border border-green-200 rounded-lg">
            <div className="flex items-start">
              <Check className="flex-shrink-0 mt-0.5 mr-3 w-6 h-6 text-green-600" />
              <div>
                <h3 className="mb-2 font-semibold text-green-800 text-lg">
                  Plan Preselected
                </h3>
                <p className="text-green-700">
                  Your memorial has been created and a plan has been
                  preselected. You can proceed with payment or choose a
                  different plan below.
                </p>
              </div>
            </div>
          </div>
      ...
      {/* Plans Grid */}
      <div className="gap-6 md:gap-8 grid lg:grid-cols-3">
        {plans?.map((plan, index) => {
          ...
          const selectedDurationOption = plan.durationOptions?.find(
            (opt) =>
              opt.duration ===
                (promoState?.selectedDuration ||
                  plan.defaultDuration ||
                  '1_month') && opt.isActive
          );
          ...
          return (
            ...
            >
              ...
              >
                {plan.isPopular && (
                  <Badge className="-top-3 left-1/2 z-10 absolute bg-[#547455] px-4 py-1 text-white -translate-x-1/2 transform">
                    {translations?.badgePopular || 'Popular'}
                  </Badge>
                )}
                {preselectedPlanId === plan._id && (
                  <Badge className="-top-3 left-1/2 z-10 absolute bg-blue-500 px-4 py-1 text-white -translate-x-1/2 transform">
                    Preselected
                  </Badge>
                )}
                  ...
                  {plan.durationOptions && plan.durationOptions.length > 0 && (
                    <div className="mt-4">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          handleDurationChange(plan._id, value)
                        }
                      >
                                ...
                                <div className="flex justify-between items-center w-full">
                                  <span>
                                    {translations?.durations?.[
                                      option.duration as keyof typeof translations.durations
                      ...
                      <div>
                        ...
                        <span className="text-gray-600">
                          ...
                          {translations?.durations?.[
                            (promoState?.selectedDuration ||
                              plan.defaultDuration ||
                              '1_month') as keyof typeof translations.durations
                          ] ||
                            (
                              promoState?.selectedDuration ||
                              plan.defaultDuration ||
                        ...
                        <Badge
                          variant="outline"
                          className="bg-green-50 mt-1 border-green-200 text-green-700"
                        >
                ...
                <CardContent className="flex-grow pt-0">
                  <ul className="space-y-3 mb-6">
                      ...
                      text={
                        plan.maxPhotos >= 999
                          ? plansTranslations?.[plan.planType]?.features
                              ?.photoUploads ||
                            translations?.features?.unlimitedPhotos ||
                            'Unlimited Photo Uploads'
                          : plansTranslations?.[plan.planType]?.features
                              ?.photoUploads ||
                            translations?.features?.photoUploads?.replace(
                              /{count}/g,
                              plan.maxPhotos.toString()
                            ) ||
                    ...
                    <FeatureListItem
                      ...
                      text={
                        plansTranslations?.[plan.planType]?.features
                          ?.slideshow ||
                        translations?.features?.slideshow ||
                        'Photo Slideshow'
                      }
                    />
                    <FeatureListItem
                      ...
                      text={
                        ...
                        translations?.features?.videoUploads?.replace(
                          /{duration}/g,
                          plan.maxVideoDuration.toString()
                        ) ||
                    ...
                    {plan.features.map((feature) => {
                      ...
                      if (planTranslations) {
                        ...
                        if (
                          ...
                          feature.text.toLowerCase().includes('document upload')
                        ) {
                          translatedText =
                          ...
                          feature.text.toLowerCase().includes('photo') &&
                          !feature.text.toLowerCase().includes('slideshow')
                        ) {
                          translatedText =
                            planTranslations.photoUploads || feature.text;
                        }
                      }
                  ...
                  <div className="pt-4 border-t">
                    <div className="flex items-center mb-2">
                      <Tag className="mr-2 w-4 h-4 text-[#243b31]" />
                      <span className="font-medium text-sm">
                        {translations?.promoCode?.title || 'Promo Code'}
                      </span>
                    </div>
                    ...
                    {promoState?.appliedPromo ? (
                        ...
                        <div className="flex justify-between items-center">
                          <div>
                            <p className="font-medium text-green-800 text-sm">
                              {translations?.promoCode?.applied || 'Applied'}:{' '}
                              {promoState.appliedPromo.code}
                            </p>
                            <p className="text-green-600 text-xs">
                              {promoState.appliedPromo.discountType ===
                              'percentage'
                                ...
                                : '100% off - FREE'}
                              {promoState.appliedPromo.appliesToPlan &&
                                ' (Plan specific)'}
                            </p>
                          </div>
                          <Button
                      ...
                      <div className="space-y-2">
                        <div className="flex gap-2">
                          <Input
                            placeholder={
                              translations?.promoCode?.placeholder ||
                              'Enter promo code'
                            }
                ...
                <CardFooter className="pt-0">
                  <Button
                    className="bg-[#243b31] hover:bg-green-700 py-3 w-full text-white text-lg"
                    onClick={() =>
                      memorialId
                        ? handleSelectPlan(plan._id)
                        : clickHandler(plan._id)
                    }
                    disabled={
                      isProcessing === plan._id ||
                      Boolean(
                        promoState?.appliedPromo?.appliesToPlan &&
                          promoState.appliedPromo.appliesToPlan !== plan._id
                      )
                    }
                  >
                    {isProcessing === plan._id
                      ? translations?.cta?.processing || 'Processing...'
                      : promoState?.appliedPromo?.discountType === 'free' &&
                        doesPromoApplyToPlan(plan._id, promoState.appliedPromo)
                      ? 'Get For Free'
                      : (plan.planType === 'minimal' &&
                          (translations?.cta?.getStarted || 'Get Started')) ||
                        (plan.planType === 'premium' &&
                          (translations?.cta?.goPremium || 'Go Premium')) ||
                        (plan.planType === 'medium' &&
                          (translations?.cta?.medium || 'Choose Medium')) ||
                        plan.ctaButtonText ||
...
const FeatureListItem = ({
  ...
  <li className="flex items-start space-x-3">
    {included ? (
      <Check className="flex-shrink-0 mt-0.5 w-5 h-5 text-green-500" />
    ) : (
      <X className="flex-shrink-0 mt-0.5 w-5 h-5 text-red-400" />
    )}
    <span
      className={`text-sm ${
